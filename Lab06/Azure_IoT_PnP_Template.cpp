...
#define OLED_SPLASH_MESSAGE "Smart Pot"
...
void azure_pnp_init() {
    ...
    display.print(OLED_SPLASH_MESSAGE);
    display.display();
    delay(2000);
    display.clearDisplay();
    display.display();
...
int azure_pnp_handle_command_request(azure_iot_t* azure_iot, command_request_t command)
{
  ...
  if (az_span_is_content_equal(command.command_name, COMMAND_NAME_TOGGLE_LED_1))
  {
    ...
    led1_on = !led1_on;
    LogInfo("Water Pump State: %s", (led1_on ? "ON" : "OFF"));
    ...
    size_t* payload_buffer_length)
{
  ...
  // Send moisture sensor data
  humidity = (sensorValue / 4095.0) * 100.0;
